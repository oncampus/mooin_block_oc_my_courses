...
jQuery.fn.motionCaptcha || (function($) {
	...
	$.fn.motionCaptcha = function(options) {
		...
		return this.each(function() {
			...
			// Set random shape
			var t = $('#formular').data("lastShape");
		
			 
			 var unsame = false;
			 var fin = 0;
				var rr = Math.floor(Math.random() * (opts.shapes.length) );
		 while (unsame == false) {
		 		var rr = Math.floor(Math.random() * (opts.shapes.length) );
		 		if (rr != t) {
		 			fin = rr;
			...
			console.log("last = "+t+" new = "+rr);
		rr = fin;
			$('#formular').data("lastShape",rr);
			
			$canvas.addClass( opts.shapes[rr] );
	...
	$.fn.motionCaptcha.defaults = {
		...
		errorMsg: 'Bitte versuche es nochmal!',
		successMsg: 'Captcha gelöst, Danke!',
		
		// This message is displayed if the user's browser doesn't support canvas:
		noCanvasMsg: "Dein Browser kann keine Vanvas, bitte verwende einen aktuellen Browser!",
